...
namespace BusinessLogicLayer.Handler.InvoiceHandler
{
    public class CreateInvoiceHandle : IRequestHandler<CreateInvoiceHandleRequest, CreateInvoiceHandleResponse>
    {
        ...
        public async Task<CreateInvoiceHandleResponse> Handle(CreateInvoiceHandleRequest request, CancellationToken cancellationToken)
        {
            ...
            else if (_currentRepository.Find(request.CurrentId) == null)
                message = "Kasa/Banka bulunamadı.";
            else if (_customerSupplierRepository.Find(request.CustomerSupplierId) == null)
            ...
            var invoice = new Invoice
            {
                ...
                No = new Random().Next(1000, 9999)//no otomatik oluşturdum
            };
